...
_user_list = sg.Listbox(
    values=users_ctr.transform_users(lambda user: user.nick),
    expand_y=True,
    size=(25, 10),
    background_color=theme.BG_BASE,
    no_scrollbar=True,
...
screen_layout = [
    ...
    [
        ...
        _csg.horizontal_spacer(
            width=theme.scale(200), background_color=theme.BG_BASE
        ),
        sg.Column(
            _button_layout,
...
def enable_selection() -> None:
    ...
    _current_user.update(
        f'Selecionado: {_user_list.get()[0]}',
        font=(theme.FONT_FAMILY, theme.H3_SIZE)
    )
    users_ctr.user = _user_list.get()[0]
...
def update_user_list() -> None:
    ...
    _user_list.update(values=users_ctr.transform_users(lambda user: user.nick))
