...
_input_age = sg.Input(
    default_text=user_ctr.user.age,
    size=(20, 1),
    background_color=theme.BG_BASE,
    font=_font,
    text_color=theme.TEXT_ACCENT,
...
_input_gender = sg.Input(
    default_text=user_ctr.user.gender,
    size=(20, 1),
    background_color=theme.BG_BASE,
    font=_font,
    text_color=theme.TEXT_ACCENT,
...
def config_layout() -> list[list[Any]]:
    ...
    config_layout = [
        ...
        [
            ...
            _csg.horizontal_spacer(
                theme.scale(200), background_color=theme.BG_BASE
            ),
            _btn_save,
        ]
    ]
...
def update_user() -> None:
    ...
    user_ctr.user.age = int(_input_age.get())
    user_ctr.user.gender = _input_gender.get()


def refresh_inputs() -> None:
    """Puts in the inputs widgets the information of the current user."""
    _input_nick.update(value=user_ctr.user.nick)
    _input_age.update(value=str(user_ctr.user.age))
    _input_gender.update(value=user_ctr.user.gender)
