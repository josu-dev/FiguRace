...
def load_theme(src_path: str, theme_name: str) -> ThemeDefinition:
    theme_path = path.join(src_path, 'database', 'data', 'theme.json')
    with open(theme_path, mode='r', encoding='utf-8') as file:
        theme = json.load(file)
        return theme['Blue-Like']


class Theme:
    def __init__(self, src_path: str, theme_name: str):
        theme_settings = load_theme(src_path, theme_name)
...
theme = Theme(SRC_PATH, 'Blue-Like')